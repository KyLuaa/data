...
if not isInArray({"list", "exit", "off", "on"}, param) and not param or param == "" then
    ...
    doPlayerSendTextMessage(cid, 20, "Comandos para cast (abrir cast) -> !cast on , (fexar cast) -> !cast off, (ver cast player online) !cast list, (sair do cast que voce ta) !cast exit.")
    return true
end
...
if param == "list" then
    ...
    if #whoCasted() < 1 then
    doSendMagicEffect(getThingPos(cid), 2)
    doPlayerSendTextMessage(cid, 20, "Nenhum Cast Aberto, Tente Novamente mas tarde.")
    return true
end
